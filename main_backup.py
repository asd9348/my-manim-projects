...
class working2(MovingCameraScene):
    ...
    def construct(self):
        ...
        wind = SVGMobject('svgs/wind.svg').flip(axis=Y_AXIS).next_to(dot_eq.get_center(), -hole_1_unit, buff=0).rotate(45 * DG).set_color(
            BLUE_A).scale(0.3).set_stroke(width=3)
        ...
        wind = SVGMobject('svgs/wind.svg').next_to(dot_eq_2.get_center(), -hole_2_unit, buff=0).rotate(45 * DG).set_color(BLUE_A).scale(
            0.3).set_stroke(width=3)
...
class working2(MovingCameraScene):
    def construct(self):
        ...
        self.play(Write(delta_p_0))
        self.add(index_labels(delta_p_0))

        axes = Axes(x_range=[ 0, 1 ],
                    y_range=[ 0, 1, 0.25 ],
                    x_length=6,
                    y_length=6,
                    tips=False)
