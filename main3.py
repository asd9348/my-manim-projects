...
class working2(Scene):
    def construct(self):
        ...
        text = Tex(
            r'\setchemfig{atom sep=2em,bond style={line width=20pt,red,dash pattern=on 10pt off 10pt}}\chemname{\chemfig{*6((=O)-N(-)-(*5(-N=-N(-)-))=-(=O)-N(-)-)}}{Ethanal}',
            tex_template=template1,)
        text = Tex(
            r'\chemfig[bond style={line width=10pt,red}]{A-B=C>|D<E>:F}',
            tex_template=template1)


        self.add(Square(20, fill_opacity=1, fill_color=WHITE))
        self.play(Create(SVGMobject('chem.svg').scale(3)))
...
class threedspace(ThreeDScene):
    def construct(self):
        ...
        dot = Sphere(color=RED, radius=0.1).move_to(axes.c2p(0, 0, 0))

        origin = axes.x_axis.get_start() + OUT * (0 - axes.z_range[ 0 ]) * axes.z_length / (abs(axes.z_range[ 0 ]) + abs(axes.z_range[ 1 ]))

        # dot_1 = Sphere(color=BLUE, radius=0.1).move_to(axes.x_axis.number_to_point(axes._origin_shift([ axes.x_axis.x_min, axes.x_axis.x_max ]))+np.array([0,0,1]))
        dot_1 = Sphere(color=BLUE, radius=0.1).move_to(my_c2p(axes, 0.5, 0.5, 0.5))
        # print(my_c2p(axes, 0.5, 0, -0.5))
        self.play(Create(dot))
