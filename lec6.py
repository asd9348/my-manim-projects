...
class Lec6(Scene):
    def construct(self):
        ...
        graph = plane.get_graph(lambda x: np.pi * x ** 2, x_min=0, x_max=2, color=GREEN)
        graph_lab = MathTex("A(r)=\pi{r}^{2}").set_width(1).next_to(graph, UR, buff=0.2)
        deriv = plane.get_graph(lambda x: 2 * np.pi * x)

        title = Tex("Area Graph").scale(0.6).next_to(plane, UP, buff=0.5)
        und = Underline(title)
        ...
        circ = always_redraw(lambda:
                             Circle(fill_color=GREEN, fill_opacity=0.75,
        ...
        area_text = Tex("area = ").scale(0.7).shift(DOWN * 2 + RIGHT)
        ...
        perim_text = Tex("perimeter = ").scale(0.7).next_to(area_text, DOWN, buff=0.75)
